...
class BaseCollection:
    ...
    def get_filtered(self, filters: Iterable[ET.Element],
                     skip_time_prefilter: bool = False
        ...
        When ``skip_time_prefilter`` is True, the time-range pre-filter
        optimization is disabled. This is needed when the collection has
        a C:calendar-timezone property set, as cached item time ranges
        ...
        tag, start, end, simple = radicale_filter.simplify_prefilters(
            filters, self.tag)
        logger.debug("TRACE/STORAGE/get_filtered: prefilter tag=%s start=%s end=%s simple=%s skip_time=%s",
                     tag, format_ut(start), format_ut(end), simple, skip_time_prefilter)
        for item in self.get_all():
            ...
            yield item, simple and not skip_time_prefilter and (start <= istart or iend <= end)
...
class BaseStorage:
    ...
    def move_collection(self, collection: BaseCollection, to_path: str) -> None:
        """Move a collection to a new path.

        ``collection`` is the collection to move.

        ``to_path`` is the target path. The parent collection must exist.
        If a collection already exists at the target path, it will be
        replaced (per RFC 4918 §9.9).
