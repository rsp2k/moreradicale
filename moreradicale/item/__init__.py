...
from moreradicale import storage  # noqa:F401
...
# vobject's VAvailability doesn't include these by default, so we patch them in
# This allows proper parsing of recurring availability rules
if hasattr(vobject_icalendar, 'VAvailability'):
    va_children = vobject_icalendar.VAvailability.knownChildren
    if 'RRULE' not in va_children:
        va_children['RRULE'] = (0, None, None)
    if 'RDATE' not in va_children:
        va_children['RDATE'] = (0, None, None)
    if 'EXDATE' not in va_children:
        va_children['EXDATE'] = (0, None, None)
    if 'EXRULE' not in va_children:
        va_children['EXRULE'] = (0, None, None)
...
def check_and_sanitize_items(
    ...
    if tag == "VCALENDAR":
        ...
        for component in vobject_item.components():
            ...
            # RFC 5545 §3.6.4: VFREEBUSY is a valid calendar component
            if component.name in ("VTODO", "VEVENT", "VJOURNAL", "VAVAILABILITY", "VFREEBUSY"):
        ...
        for component in vobject_item.components():
            ...
            # RFC 7953: VAVAILABILITY is a valid calendar component
            # RFC 5545 §3.6.4: VFREEBUSY is a valid calendar component
            if component_name not in ("VTODO", "VEVENT", "VJOURNAL", "VAVAILABILITY", "VFREEBUSY"):
                continue
            component_uid = get_uid(component)
            ...
            # RFC 5545 §3.6.4: VFREEBUSY doesn't support recurrence rules
            if component_name not in ("VAVAILABILITY", "VFREEBUSY"):
                try:
                    component.rruleset
...
def get_uid_from_object(vobject_item: vobject.base.Component) -> str:
    ...
    if vobject_item.name == "VCALENDAR":
        ...
        # RFC 5545 §3.6.4: VFREEBUSY component
        if hasattr(vobject_item, "vfreebusy"):
            return get_uid(vobject_item.vfreebusy)
        # RFC 7953: VAVAILABILITY component
        if hasattr(vobject_item, "vavailability"):
            return get_uid(vobject_item.vavailability)
