...
def _auto_version_item(configuration, path: str, user: str,
                       is_update: bool) -> None:
    """Create automatic git commit after PUT if auto-versioning is enabled.

    Args:
        configuration: Radicale configuration
        path: Item path (e.g., "user/calendar.ics/event.ics")
        user: User performing the operation
        is_update: True if updating existing item, False if creating new
    """
    # Check if versioning and auto-versioning are enabled
    if not configuration.get("storage", "versioning"):
        return

    auto_mode = configuration.get("storage", "versioning_auto")
    ...
    try:
        ...
        if not git_writer.is_available():
            return

        relative_path = f"collection-root/{path.strip('/')}"
        ...
        message = f"AUTO-VERSION: {action} {item_name}"

        sha = git_writer.create_version(
            relative_path, effective_user, user_email, message
        )
        if sha:
...
PRODID = u"-//moreradicale//NONSGML Version " + utils.package_version("moreradicale") + "//EN"
        ...
        with self._storage.acquire_lock("w", user, path=path, request="PUT"):
            ...
            if write_whole_collection:
                ...
                try:
                    ...
                    # Auto-versioning: create git commit if enabled
                    _auto_version_item(
                        self.configuration, path, user,
                        is_update=(replaced_item is not None)
                    )
