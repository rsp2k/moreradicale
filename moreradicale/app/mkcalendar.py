...
class ApplicationPartMkcalendar(ApplicationBase):
    ...
    def do_MKCALENDAR(self, environ: types.WSGIEnviron, base_prefix: str,
        ...
        if reserved is not None:
            logger.warning("Refused MKCALENDAR carrying server-managed "
                           "property %r on %r by %r", reserved, path, user)
        ...
        try:
            ...
            logger.warning(
                "Bad MKCALENDAR request on %r: %s", path, e, exc_info=True)
            return httputils.BAD_REQUEST
        # RFC 4791 §5.3.2: C:calendar-timezone property is stored and used
        # by REPORT operations to interpret floating times in calendar items.
        # The property value (a VTIMEZONE component) is passed through props.
