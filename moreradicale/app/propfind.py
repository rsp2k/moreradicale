...
def _get_git_metadata_reader(configuration: 'config.Configuration'):
    """Lazy-load GitMetadataReader to avoid import overhead when not used."""
    global _git_metadata_reader
    if _git_metadata_reader is None:
        if configuration.get("storage", "versioning"):
            try:
                ...
                _git_metadata_reader = GitMetadataReader(collection_root, max_history)
            except ImportError:
                _git_metadata_reader = False  # Mark as unavailable
        else:
            _git_metadata_reader = False
    return _git_metadata_reader if _git_metadata_reader else None
...
def xml_propfind_response(
    ...
    if propname or allprop:
        ...
        # RFC 3253 versioning properties for items (not collections)
        if not is_collection:
            if (configuration.get("storage", "versioning") and
                    configuration.get("storage", "versioning_include_in_allprop")):
                props.append(xmlutils.make_clark("D:version-history"))
                props.append(xmlutils.make_clark("D:checked-in"))
                props.append(xmlutils.make_clark("D:version-name"))
                props.append(xmlutils.make_clark("D:creator-displayname"))
                props.append(xmlutils.make_clark("D:label-name-set"))
                props.append(xmlutils.make_clark("D:activity-set"))
    ...
    for tag in props:
        ...
        # RFC 6638 scheduling property handlers
            ...
            if configuration.get("push", "enabled") and is_leaf:
                from moreradicale.push.subscription import generate_pushkey
            ...
            # RFC 3253 versioning reports
            if not is_collection and configuration.get("storage", "versioning"):
                reports.append("D:version-tree")
        ...
        # RFC 8607 managed attachment properties
            ...
            if tag == xmlutils.make_clark("D:getcontenttype"):
                ...
                if isinstance(item, storage.BaseCollection) and not collection.is_principal:
                    ...
                    is404 = True
            elif tag == xmlutils.make_clark("RADICALE:notifications"):
                # Expose stored sharing-notification metadata so the web UI
                # can render an inbox without parsing notification XML.
                # Each notification subcollection under /{user}/notifications/
                # carries a JSON blob describing the invite/reply/revocation.
                notif_json = collection.get_meta("RADICALE:notifications")
                if notif_json:
        ...
        # Not for collections
            ...
            pass
        # RFC 3253 versioning properties for items
        elif tag == xmlutils.make_clark("D:version-history"):
            # Link to the version history resource
            assert not isinstance(item, storage.BaseCollection)
            git_reader = _get_git_metadata_reader(configuration)
            if git_reader and git_reader.is_available() and item.href:
                # Version history URL: /.versions/{collection}/{item}/
                version_history_url = f"/.versions/{collection.path}/{item.href}/"
                child_element = ET.Element(xmlutils.make_clark("D:href"))
                child_element.text = xmlutils.make_href(base_prefix, version_history_url)
                element.append(child_element)
            else:
                is404 = True
        elif tag == xmlutils.make_clark("D:checked-in"):
            # Current version URL (latest git commit affecting this item)
            assert not isinstance(item, storage.BaseCollection)
            git_reader = _get_git_metadata_reader(configuration)
            if git_reader and git_reader.is_available() and item.href:
                # Get relative path within storage folder
                relative_path = f"collection-root/{collection.path}/{item.href}"
                version = git_reader.get_current_version(relative_path)
                if version:
                    # Checked-in URL: /.versions/{collection}/{item}/{sha}
                    checked_in_url = f"/.versions/{collection.path}/{item.href}/{version.short_sha}"
                    child_element = ET.Element(xmlutils.make_clark("D:href"))
                    child_element.text = xmlutils.make_href(base_prefix, checked_in_url)
                    element.append(child_element)
                else:
                    is404 = True
            else:
                is404 = True
        elif tag == xmlutils.make_clark("D:version-name"):
            # Human-readable version identifier (short git SHA)
            assert not isinstance(item, storage.BaseCollection)
            git_reader = _get_git_metadata_reader(configuration)
            if git_reader and git_reader.is_available() and item.href:
                relative_path = f"collection-root/{collection.path}/{item.href}"
                version = git_reader.get_current_version(relative_path)
                if version:
                    element.text = version.version_name
                else:
                    is404 = True
            else:
                is404 = True
        elif tag == xmlutils.make_clark("D:creator-displayname"):
            # Who made the last change (git commit author)
            assert not isinstance(item, storage.BaseCollection)
            git_reader = _get_git_metadata_reader(configuration)
            if git_reader and git_reader.is_available() and item.href:
                relative_path = f"collection-root/{collection.path}/{item.href}"
                version = git_reader.get_current_version(relative_path)
                if version:
                    element.text = version.author
                else:
                    is404 = True
            else:
                is404 = True
        elif tag == xmlutils.make_clark("D:label-name-set"):
            # RFC 3253 §8.1: Set of labels that select this version
            assert not isinstance(item, storage.BaseCollection)
            git_reader = _get_git_metadata_reader(configuration)
            if git_reader and git_reader.is_available() and item.href:
                relative_path = f"{collection.path}/{item.href}"
                version = git_reader.get_current_version(relative_path)
                if version and version.sha:
                    # Get labels for this version's commit
                    labels = git_reader.get_labels_for_commit(version.sha, relative_path)
                    if labels:
                        # Build label-name-set with child label-name elements
                        for label in labels:
                            label_elem = ET.SubElement(element, xmlutils.make_clark("D:label-name"))
                            label_elem.text = label
                    # Empty label-name-set is valid (no labels)
                    # Don't mark as 404
                else:
                    is404 = True
            else:
                is404 = True
        elif tag == xmlutils.make_clark("D:activity-set"):
            # RFC 3253 §13.2: Set of activities associated with this resource
            assert not isinstance(item, storage.BaseCollection)
            if configuration.get("storage", "versioning") and item.href:
                try:
                    ...
                    if activity_ids:
                        # Build activity-set with child href elements
                        for activity_id in activity_ids:
                            href_elem = ET.SubElement(element, xmlutils.make_clark("D:href"))
                            href_elem.text = f"/.activities/{activity_id}"
                    # Empty activity-set is valid (no activities)
                    # Don't mark as 404
                except ImportError:
                    is404 = True
            else:
