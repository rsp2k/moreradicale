...
def _limit_recurrence_set(item_text: str, start: datetime.datetime,
                          end: datetime.datetime) -> str:
    """Apply limit-recurrence-set to a recurring calendar component.

    RFC 4791 §9.6.6: Limit the returned recurrence instances to only those
    that overlap the specified time range. The RRULE is preserved but
    overridden instances (RECURRENCE-ID) outside the range are removed.

    Args:
        item_text: Serialized iCalendar data with recurring event
        start: Start of the time range (UTC)
        end: End of the time range (UTC)

    Returns:
        Modified iCalendar data with filtered overrides
    """
    try:
        cal = vobject.readOne(item_text)
    except Exception as e:
        logger.warning("Failed to parse calendar for limit-recurrence-set: %s", e)
        return item_text

    if not hasattr(cal, 'vevent_list') or not cal.vevent_list:
        return item_text

    # Find the master event (the one with RRULE, no RECURRENCE-ID)
    master_event = None
    overridden_events = []
    duration = None
    ...
    for vevent in cal.vevent_list:
        if hasattr(vevent, 'recurrence_id'):
            overridden_events.append(vevent)
        else:
            master_event = vevent
            # Calculate duration for checking overlap
    ...
    if not master_event:
        # No master event means no recurrence to limit
        return item_text

    # Filter overridden instances to only those overlapping the time range
    filtered_overrides = []
    for vevent in overridden_events:
        ...
        if hasattr(vevent, 'dtend'):
            ...
            # No duration info, assume same as start (point in time)
            dtend = dtstart
...
def _limit_freebusy_set(item_text: str, start: datetime.datetime,
                        end: datetime.datetime) -> str:
    """Apply limit-freebusy-set to VFREEBUSY component.

    RFC 4791 §9.6.7: Limit the returned FREEBUSY values to only those
    that intersect the specified time range.

    Args:
        item_text: Serialized iCalendar data containing VFREEBUSY
        start: Start of the time range (UTC)
        end: End of the time range (UTC)

    Returns:
        Modified iCalendar data with filtered FREEBUSY periods
    """
    try:
        cal = vobject.readOne(item_text)
    except Exception as e:
        logger.warning("Failed to parse VFREEBUSY for limit-freebusy-set: %s", e)
        return item_text

    if not hasattr(cal, 'vfreebusy'):
        return item_text

    vfb = cal.vfreebusy

    # Filter FREEBUSY periods
    freebusy_list = vfb.contents.get("freebusy", [])
    if not freebusy_list:
        return item_text

    # Collect filtered periods by FBTYPE
    filtered_by_type: Dict[str, List[Tuple[datetime.datetime, datetime.datetime]]] = {}

    for fb_prop in freebusy_list:
        fb_type = fb_prop.params.get('FBTYPE', ['BUSY'])[0]
        ...
        for period in periods:
            ...
            # Check if period intersects with the requested range
            if period_start < end and period_end > start:
                if fb_type not in filtered_by_type:
                    filtered_by_type[fb_type] = []
                filtered_by_type[fb_type].append((period_start, period_end))

    # Remove old FREEBUSY properties
    if "freebusy" in vfb.contents:
        del vfb.contents["freebusy"]

    # Add filtered FREEBUSY periods
    for fb_type, periods in filtered_by_type.items():
...
def _coalesce_periods(periods: List[Tuple[datetime.datetime, datetime.datetime]]
                      ) -> List[Tuple[datetime.datetime, datetime.datetime]]:
    """Coalesce overlapping or adjacent time periods.

    RFC 4791 §7.10: The server SHOULD coalesce the FREEBUSY information
    into a single VFREEBUSY component with overlapping/adjacent periods merged.
...
def free_busy_report(base_prefix: str, path: str, xml_request: Optional[ET.Element],
    ...
    # Build filters for free-busy query - check both VEVENT and VFREEBUSY
    # RFC 4791 §7.10: Free-busy queries should consider stored VFREEBUSY components
    filters = []
    for comp_type in ('VEVENT', 'VFREEBUSY'):
        comp_cf_element = ET.Element(xmlutils.make_clark("C:comp-filter"),
                                     attrib={'name': comp_type})
        # Clone time_range_element since it can only have one parent
        time_range_copy = ET.Element(time_range_element.tag, time_range_element.attrib)
        comp_cf_element.append(time_range_copy)
        vcalendar_cf_element = ET.Element(xmlutils.make_clark("C:comp-filter"),
                                          attrib={'name': 'VCALENDAR'})
        vcalendar_cf_element.append(comp_cf_element)
    ...
    # RFC 4791 §5.3.2: Get calendar-timezone for floating time interpretation
    # Must be done BEFORE retrieving items since it affects pre-filter optimization
    calendar_tz_prop = collection.get_meta("C:calendar-timezone")
    calendar_tz = parse_calendar_timezone(calendar_tz_prop)
    skip_time_prefilter = calendar_tz is not None
    if calendar_tz:
        logger.debug("Using calendar-timezone for free-busy filtering: %s", calendar_tz)
        set_default_floating_timezone(calendar_tz)

    # First pull from storage
    # When calendar-timezone is set, skip time pre-filter as cached timestamps
    # use UTC but should be interpreted with the calendar's timezone.
    retrieved_items = list(collection.get_filtered(filters, skip_time_prefilter=skip_time_prefilter))

    # !!! Don't access storage after this !!!
    unlock_storage_fn()

    # RFC 4791 §7.10: Collect periods by FBTYPE for coalescing
    # Periods are grouped by type so overlapping BUSY periods merge with BUSY,
    # BUSY-TENTATIVE with BUSY-TENTATIVE, etc.
    periods_by_fbtype: Dict[str, List[Tuple[datetime.datetime, datetime.datetime]]] = {}
    dtstamp_value = datetime.datetime.now(datetime.timezone.utc)
    ...
    try:
        while retrieved_items:
            ...
            if not filter_matched:
                try:
                    # Test against all filters (VEVENT and VFREEBUSY)
                    matched_any = any(test_filter(collection_tag, item, f) for f in filters)
                    if not matched_any:
                        continue
            ...
            if item.component_name == 'VEVENT':
                ...
                # Collect periods for later coalescing
                if fbtype not in periods_by_fbtype:
                    periods_by_fbtype[fbtype] = []
                periods_by_fbtype[fbtype].extend(occurrences)

                # Use first event's dtstamp if available
                event_dtstamp = getattr(item.vobject_item.vevent, 'dtstamp', None)
                if event_dtstamp:
                    dtstamp_value = event_dtstamp.value

            elif item.component_name == 'VFREEBUSY':
                # RFC 5545 §3.6.4: Pass through stored VFREEBUSY data
                # Extract FREEBUSY periods from the stored component
                vfreebusy = item.vobject_item.vfreebusy
                dtstamp = getattr(vfreebusy, 'dtstamp', None)
                if dtstamp:
                    dtstamp_value = dtstamp.value

                # Get DTSTART/DTEND if present
                dtstart = getattr(vfreebusy, 'dtstart', None)
                dtend = getattr(vfreebusy, 'dtend', None)

                # Check FREEBUSY property periods
                freebusy_list = vfreebusy.contents.get("freebusy", [])
                if freebusy_list:
                    for fb_prop in freebusy_list:
                        # Get FBTYPE parameter (defaults to BUSY)
                        fb_type = fb_prop.params.get('FBTYPE', ['BUSY'])[0]
                        if fb_type not in periods_by_fbtype:
                            periods_by_fbtype[fb_type] = []

                        periods = fb_prop.value if isinstance(fb_prop.value, list) else [fb_prop.value]
                        for period in periods:
                            if hasattr(period, 'start') and hasattr(period, 'end'):
                                periods_by_fbtype[fb_type].append((period.start, period.end))
                            elif hasattr(period, '__iter__') and len(period) >= 2:
                                start = period[0]
                                if isinstance(period[1], datetime.timedelta):
                                    end = period[0] + period[1]
                                else:
                                    end = period[1]
                                periods_by_fbtype[fb_type].append((start, end))
                elif dtstart and dtend:
                    # No FREEBUSY periods, but has DTSTART/DTEND - treat as single busy period
                    if 'BUSY' not in periods_by_fbtype:
                        periods_by_fbtype['BUSY'] = []
                    periods_by_fbtype['BUSY'].append((dtstart.value, dtend.value))

        # RFC 4791 §7.10: Coalesce overlapping periods and build output
        cal = vobject.iCalendar()

        # Process each FBTYPE group, coalesce, and add to output
        for fbtype, periods in periods_by_fbtype.items():
            coalesced = _coalesce_periods(periods)
            for start, end in coalesced:
                vfb = cal.add('vfreebusy')
                vfb.add('dtstamp').value = dtstamp_value
        ...
        # RFC 4791 §5.3.2: Clear the calendar-timezone after filtering
        set_default_floating_timezone(None)
...
def xml_report(base_prefix: str, path: str, xml_request: Optional[ET.Element],
    ...
    # RFC 4791 §9.6.6: limit-recurrence-set limits recurrence overrides in response
    limit_recurrence_set = root.find(".//" + xmlutils.make_clark("C:limit-recurrence-set"))
    # RFC 4791 §9.6.7: limit-freebusy-set limits FREEBUSY periods in response
    limit_freebusy_set = root.find(".//" + xmlutils.make_clark("C:limit-freebusy-set"))
    ...
    skip_time_prefilter = calendar_tz is not None
    if calendar_tz:
        logger.debug("Using calendar-timezone for filtering: %s", calendar_tz)
        set_default_floating_timezone(calendar_tz)
    ...
    retrieved_items = list(retrieve_items(
        ...
        skip_time_prefilter=skip_time_prefilter))
    collection_tag = collection.tag
        ...
        while retrieved_items:
            ...
            for prop in props:
                ...
                if prop.tag == xmlutils.make_clark("D:getcontenttype"):
                    ...
                    # RFC 4791 §9.6.7: Apply limit-freebusy-set to VFREEBUSY items
                    if (limit_freebusy_set is not None and
                            item.component_name == 'VFREEBUSY' and
                            prop.tag == xmlutils.make_clark("C:calendar-data")):
                        lfb_start_str = limit_freebusy_set.get('start')
                        lfb_end_str = limit_freebusy_set.get('end')

                        if lfb_start_str is None or lfb_end_str is None:
                            return client.FORBIDDEN, \
                                xmlutils.webdav_error("C:valid-calendar-data")

                        lfb_start = datetime.datetime.strptime(
                            lfb_start_str, DT_FORMAT_TIMESTAMP
                        ).replace(tzinfo=datetime.timezone.utc)
                        lfb_end = datetime.datetime.strptime(
                            lfb_end_str, DT_FORMAT_TIMESTAMP
                        ).replace(tzinfo=datetime.timezone.utc)

                        element.text = _limit_freebusy_set(element.text, lfb_start, lfb_end)

                    # RFC 4791 §9.6.6: Apply limit-recurrence-set to recurring events
                    # Note: limit-recurrence-set and expand are mutually exclusive per DTD
                    if (limit_recurrence_set is not None and
                            item.component_name == 'VEVENT' and
                            prop.tag == xmlutils.make_clark("C:calendar-data")):
                        lrs_start_str = limit_recurrence_set.get('start')
                        lrs_end_str = limit_recurrence_set.get('end')

                        if lrs_start_str is None or lrs_end_str is None:
                            return client.FORBIDDEN, \
                                xmlutils.webdav_error("C:valid-calendar-data")

                        lrs_start = datetime.datetime.strptime(
                            lrs_start_str, DT_FORMAT_TIMESTAMP
                        ).replace(tzinfo=datetime.timezone.utc)
                        lrs_end = datetime.datetime.strptime(
                            lrs_end_str, DT_FORMAT_TIMESTAMP
                        ).replace(tzinfo=datetime.timezone.utc)

                        element.text = _limit_recurrence_set(element.text, lrs_start, lrs_end)

                    # Note: expand and limit-recurrence-set are mutually exclusive (RFC 4791 §9.6)
...
def retrieve_items(
        ...
        skip_time_prefilter: bool = False) -> Iterator[Tuple[radicale_item.Item, bool]]:
    """Retrieves all items that are referenced in ``hreferences`` from
       ...
       When ``skip_time_prefilter`` is True, the storage layer's time-range
       pre-filter optimization is disabled (needed for calendar-timezone).
    """
    ...
    if collection_requested:
        logger.debug("TRACE/REPORT/retrieve_items: get_filtered skip_time=%s", skip_time_prefilter)
        yield from collection.get_filtered(filters, skip_time_prefilter=skip_time_prefilter)
