...
Can be used with an external WSGI server (see ``moreradicale.application()``) or
the built-in server (see ``moreradicale.server`` module).
...
from moreradicale.app.mkactivity import ApplicationPartMkactivity
from moreradicale.app.mkcalendar import ApplicationPartMkcalendar
from moreradicale.app.mkcol import ApplicationPartMkcol
from moreradicale.app.move import ApplicationPartMove
from moreradicale.app.options import ApplicationPartOptions
from moreradicale.app.post import ApplicationPartPost
from moreradicale.app.propfind import ApplicationPartPropfind
from moreradicale.app.proppatch import ApplicationPartProppatch
from moreradicale.app.put import ApplicationPartPut
...
REQUEST_METHODS = ["CHECKIN", "CHECKOUT", "DELETE", "GET", "HEAD", "LABEL", "MKACTIVITY", "MKCALENDAR", "MKCOL", "MOVE", "OPTIONS", "POST", "PROPFIND", "PROPPATCH", "PUT", "REPORT", "UNCHECKOUT", "VERSION-CONTROL"]


class Application(ApplicationPartCheckin, ApplicationPartCheckout,
                  ApplicationPartDelete, ApplicationPartHead,
                  ApplicationPartGet, ApplicationLabelMixin,
                  ApplicationPartMkactivity, ApplicationPartMkcalendar,
                  ...
                  ApplicationPartUncheckout, ApplicationPartVersionControl,
                  ApplicationBase):
    """WSGI application."""
    ...
    def _handle_request(self, environ: types.WSGIEnviron
        ...
        # (e.g., VERSION-CONTROL -> VERSION_CONTROL)
        method_name = request_method.replace("-", "_")
        function = getattr(self, "do_%s" % method_name, None)
        if not function:
        ...
        # Handle RFC 3253 versioning virtual paths (/.versions/)
        if path.startswith("/.versions/"):
            if self.configuration.get("storage", "versioning"):
                from moreradicale.versioning import VersioningHandler
                versioning_handler = VersioningHandler(
                    self.configuration, self._storage)

                if not user:
                    # Versioning requires authentication to check access rights
                    return response(*httputils.NOT_ALLOWED)

                # Check read access to the underlying collection
                version_path = versioning_handler.parse_version_path(path)
                if version_path[0]:  # collection_path
                    collection_path = "/" + version_path[0] + "/"
                    if "r" not in self._rights.authorization(user, collection_path):
                        logger.info("Version access denied to %r for %s",
                                    collection_path, repr(user))
                        return response(*httputils.NOT_ALLOWED)
