...
This is an automated message. Please do not reply.""",
        ...
        "deleted_or_removed_from_todo_template": {
            "value": """Hello $attendee_name,
...
The following task has been updated.
    ...
    Priority: $item_priority
    Status: $item_status
    Progress: $item_percent_complete%

This is an automated message. Please do not reply.""",
            "type": str
        },
        "new_journal_template": {
            "value": """Hello $attendee_name,

A new journal entry has been shared with you.

    $item_title
    Date: $item_start_time

This is an automated message. Please do not reply.""",
            "type": str
        },
        "deleted_journal_template": {
            "value": """Hello $attendee_name,

The following journal entry has been deleted.
...
This is an automated message. Please do not reply.""",
        ...
        "updated_journal_template": {
            "value": """Hello $attendee_name,

The following journal entry has been updated.
...
MESSAGE_TEMPLATE_VARIABLES = [
    ...
    "event_end_time",
    "event_location",
    # Generic item variables (for todo/journal)
    "item_title",
    "item_start_time",
    "item_due_time",
    "item_priority",
...
def read_ics_todo(contents: str) -> Optional['Todo']:
    """
    Read the vobject item from the provided string and create a Todo.
    """
    v_cal: vobject.base.Component = vobject.readOne(contents)
    cal: Calendar = Calendar(vobject_item=v_cal)
    return cal.todo if cal.todo else None


def read_ics_journal(contents: str) -> Optional['Journal']:
    """
    Read the vobject item from the provided string and create a Journal.
    """
    v_cal: vobject.base.Component = vobject.readOne(contents)
    cal: Calendar = Calendar(vobject_item=v_cal)
    return cal.journal if cal.journal else None


def ics_contents_contains_event(contents: str) -> bool:
    """
...
def ics_contents_contains_todo(contents: str) -> bool:
    """
    Check if the ICS contents contain a todo/task.
    :param contents: The contents of the ICS file.
    :return: True if the ICS file contains a todo, False otherwise.
    """
    return read_ics_todo(contents) is not None


def ics_contents_contains_journal(contents: str) -> bool:
    """
    Check if the ICS contents contain a journal entry.
    :param contents: The contents of the ICS file.
    :return: True if the ICS file contains a journal, False otherwise.
    """
    return read_ics_journal(contents) is not None


def get_ics_component_type(contents: str) -> Optional[str]:
    """
    Determine the type of component in the ICS contents.
    :param contents: The contents of the ICS file.
    :return: 'VEVENT', 'VTODO', 'VJOURNAL', or None if not recognized.
    """
    if ics_contents_contains_event(contents):
        return 'VEVENT'
    if ics_contents_contains_todo(contents):
        return 'VTODO'
    if ics_contents_contains_journal(contents):
        return 'VJOURNAL'
    return None
...
class Todo(VComponent):
    """VTODO component wrapper for task/to-do items (RFC 5545 §3.6.2)."""

    def __init__(self,
                 vobject_item: vobject.base.Component):
        """Initialize a VTODO item."""
        super().__init__(vobject_item, "VTODO")

    @property
    def datetime_stamp(self) -> Optional[str]:
        """Return the last modification datetime of the todo."""
        return self._get_content_lines("DTSTAMP")[0].value

    @property
    def datetime_start(self) -> Optional[TimeWithTimezone]:
        """Return the start datetime of the todo."""
        _content_line = self._get_content_lines("DTSTART")[0]
        return TimeWithTimezone(_content_line) if _content_line.value else None

    @property
    def datetime_due(self) -> Optional[TimeWithTimezone]:
        """Return the due datetime of the todo."""
        _content_line = self._get_content_lines("DUE")[0]
        return TimeWithTimezone(_content_line) if _content_line.value else None

    @property
    def duration(self) -> Optional[int]:
        """Return the duration of the todo."""
        return self._get_content_lines("DURATION")[0].value

    @property
    def uid(self) -> Optional[str]:
        """Return the UID of the todo."""
        return self._get_content_lines("UID")[0].value

    @property
    def status(self) -> Optional[str]:
        """Return the status of the todo (NEEDS-ACTION, IN-PROCESS, COMPLETED, CANCELLED)."""
    ...
    def organizer(self) -> Optional[str]:
        """Return the organizer of the todo."""
        return self._get_content_lines("ORGANIZER")[0].value

    @property
    def priority(self) -> Optional[int]:
        """Return the priority of the todo (1-9, where 1 is highest)."""
        priority = self._get_content_lines("PRIORITY")[0].value
        return int(priority) if priority is not None else None

    @property
    def percent_complete(self) -> Optional[int]:
        """Return the percent complete of the todo (0-100)."""
        percent = self._get_content_lines("PERCENT-COMPLETE")[0].value
        return int(percent) if percent is not None else None

    @property
    def attendees(self) -> List[Attendee]:
        """Return a list of ATTENDEE items in the todo."""
...
class Journal(VComponent):
    """VJOURNAL component wrapper for journal entries (RFC 5545 §3.6.3)."""

    def __init__(self,
                 vobject_item: vobject.base.Component):
        """Initialize a VJOURNAL item."""
        super().__init__(vobject_item, "VJOURNAL")

    @property
    def datetime_stamp(self) -> Optional[str]:
        """Return the last modification datetime of the journal."""
        return self._get_content_lines("DTSTAMP")[0].value

    @property
    def datetime_start(self) -> Optional[TimeWithTimezone]:
        """Return the datetime of the journal entry."""
        _content_line = self._get_content_lines("DTSTART")[0]
        return TimeWithTimezone(_content_line) if _content_line.value else None

    @property
    def uid(self) -> Optional[str]:
        """Return the UID of the journal."""
        return self._get_content_lines("UID")[0].value

    @property
    def status(self) -> Optional[str]:
        """Return the status of the journal (DRAFT, FINAL, CANCELLED)."""
        return self._get_content_lines("STATUS")[0].value

    @property
    def summary(self) -> Optional[str]:
        """Return the summary of the journal."""
        return self._get_content_lines("SUMMARY")[0].value

    @property
    def description(self) -> Optional[str]:
        """Return the description/content of the journal."""
        return self._get_content_lines("DESCRIPTION")[0].value

    @property
    def organizer(self) -> Optional[str]:
        """Return the organizer of the journal."""
        return self._get_content_lines("ORGANIZER")[0].value

    @property
    def attendees(self) -> List[Attendee]:
        """Return a list of ATTENDEE items in the journal."""
        _content_lines = self._get_content_lines("ATTENDEE")
        return [Attendee(content_line=attendee) for attendee in _content_lines if attendee.value is not None]
...
class Calendar(VComponent):
    ...
    def todo(self) -> Optional[Todo]:
        """Return the VTODO item in the calendar."""
        return self._get_sub_vobjects("vtodo", Todo)[0]  # type: ignore

    @property
    def journal(self) -> Optional[Journal]:
        """Return the VJOURNAL item in the calendar."""
        return self._get_sub_vobjects("vjournal", Journal)[0]  # type: ignore
...
class MessageTemplate:
    ...
    def build_todo_message(self, todo: 'EmailTodo', from_email: str, mass_email: bool,
                           attendee: Optional[Attendee] = None) -> str:
        """Build the message body for a todo notification."""
        if mass_email:
            attendee_name = "everyone"
        else:
            assert attendee is not None, "Attendee must be provided for non-mass emails"
        ...
        priority_str = str(todo.todo.priority) if todo.todo.priority else "None"
        percent_str = str(todo.todo.percent_complete) if todo.todo.percent_complete is not None else "0"

        context = {
            "attendee_name": attendee_name,
            "from_email": from_email,
            "organizer_name": todo.todo.organizer or "Unknown Organizer",
            "item_title": todo.todo.summary or "No Title",
            "item_start_time": todo.todo.datetime_start.time_string() if todo.todo.datetime_start else "Not Set",
            "item_due_time": todo.todo.datetime_due.time_string() if todo.todo.datetime_due else "No Due Date",
            "item_priority": priority_str,
            "item_status": todo.todo.status or "NEEDS-ACTION",
            "item_percent_complete": percent_str,
            "item_location": todo.todo.location or "No Location Specified",
        }
        return self._populate_template(template=self.body, context=context)

    def build_todo_subject(self, todo: 'EmailTodo', from_email: str, mass_email: bool,
                           attendee: Optional[Attendee] = None) -> str:
        """Build the subject line for a todo notification."""
        if mass_email:
            attendee_name = "everyone"
        else:
            assert attendee is not None, "Attendee must be provided for non-mass emails"
    ...
    def build_journal_message(self, journal: 'EmailJournal', from_email: str, mass_email: bool,
                              attendee: Optional[Attendee] = None) -> str:
        """Build the message body for a journal notification."""
        if mass_email:
            attendee_name = "everyone"
        else:
            assert attendee is not None, "Attendee must be provided for non-mass emails"
        ...
        context = {
            "attendee_name": attendee_name,
            "from_email": from_email,
            "organizer_name": journal.journal.organizer or "Unknown Organizer",
            "item_title": journal.journal.summary or "No Title",
            "item_start_time": journal.journal.datetime_start.time_string() if journal.journal.datetime_start else "Not Set",
            "item_status": journal.journal.status or "FINAL",
        }
        return self._populate_template(template=self.body, context=context)

    def build_journal_subject(self, journal: 'EmailJournal', from_email: str, mass_email: bool,
                              attendee: Optional[Attendee] = None) -> str:
        """Build the subject line for a journal notification."""
        if mass_email:
            attendee_name = "everyone"
        else:
            assert attendee is not None, "Attendee must be provided for non-mass emails"
...
class EmailConfig:
    def __init__(self,
                 ...
                 # VTODO templates
                 new_or_added_to_todo_template: Optional[MessageTemplate] = None,
                 deleted_or_removed_from_todo_template: Optional[MessageTemplate] = None,
                 updated_todo_template: Optional[MessageTemplate] = None,
                 # VJOURNAL templates
                 new_journal_template: Optional[MessageTemplate] = None,
                 deleted_journal_template: Optional[MessageTemplate] = None,
                 updated_journal_template: Optional[MessageTemplate] = None):
        self.host = host
        self.port = port
        ...
        # Todo templates
        self.new_or_added_to_todo_template = new_or_added_to_todo_template
        self.deleted_or_removed_from_todo_template = deleted_or_removed_from_todo_template
        self.updated_todo_template = updated_todo_template
        # Journal templates
        self.new_journal_template = new_journal_template
        self.deleted_journal_template = deleted_journal_template
        self.updated_journal_template = updated_journal_template
    ...
    # VTODO notification methods
    def send_added_todo_email(self, attendees: List[Attendee], todo: EmailTodo) -> bool:
        """Send a notification for created todos (and/or assigning attendees)."""
        if not self.new_or_added_to_todo_template:
            logger.debug("No todo template configured, skipping notification.")
            return True
        ics_attachment = ICSEmailAttachment(file_content=todo.ics_content, file_name=f"{todo.file_name}")
        return self._prepare_and_send_todo_email(
            template=self.new_or_added_to_todo_template, attendees=attendees,
            todo=todo, ics_attachment=ics_attachment)

    def send_updated_todo_email(self, attendees: List[Attendee], todo: EmailTodo) -> bool:
        """Send a notification for updated todos."""
        if not self.updated_todo_template:
            logger.debug("No todo update template configured, skipping notification.")
            return True
        ics_attachment = ICSEmailAttachment(file_content=todo.ics_content, file_name=f"{todo.file_name}")
        return self._prepare_and_send_todo_email(
            template=self.updated_todo_template, attendees=attendees,
            todo=todo, ics_attachment=ics_attachment)

    def send_deleted_todo_email(self, attendees: List[Attendee], todo: EmailTodo) -> bool:
        """Send a notification for deleted todos."""
        if not self.deleted_or_removed_from_todo_template:
            logger.debug("No todo deletion template configured, skipping notification.")
            return True
        return self._prepare_and_send_todo_email(
            template=self.deleted_or_removed_from_todo_template, attendees=attendees,
            todo=todo, ics_attachment=None)

    # VJOURNAL notification methods
    def send_added_journal_email(self, attendees: List[Attendee], journal: EmailJournal) -> bool:
        """Send a notification for new journal entries."""
        if not self.new_journal_template:
            logger.debug("No journal template configured, skipping notification.")
            return True
        ics_attachment = ICSEmailAttachment(file_content=journal.ics_content, file_name=f"{journal.file_name}")
        return self._prepare_and_send_journal_email(
            template=self.new_journal_template, attendees=attendees,
            journal=journal, ics_attachment=ics_attachment)

    def send_updated_journal_email(self, attendees: List[Attendee], journal: EmailJournal) -> bool:
        """Send a notification for updated journal entries."""
        if not self.updated_journal_template:
            logger.debug("No journal update template configured, skipping notification.")
            return True
        ics_attachment = ICSEmailAttachment(file_content=journal.ics_content, file_name=f"{journal.file_name}")
        return self._prepare_and_send_journal_email(
            template=self.updated_journal_template, attendees=attendees,
            journal=journal, ics_attachment=ics_attachment)

    def send_deleted_journal_email(self, attendees: List[Attendee], journal: EmailJournal) -> bool:
        """Send a notification for deleted journal entries."""
        if not self.deleted_journal_template:
            logger.debug("No journal deletion template configured, skipping notification.")
            return True
        return self._prepare_and_send_journal_email(
            template=self.deleted_journal_template, attendees=attendees,
            journal=journal, ics_attachment=None)
    ...
    def _prepare_and_send_todo_email(self, template: MessageTemplate, attendees: List[Attendee],
                                     todo: EmailTodo, ics_attachment: Optional[ICSEmailAttachment] = None) -> bool:
        """
        Prepare the email message(s) and send them to the attendees for a todo notification.
        :param template: The message template to use for the email.
        :param attendees: The list of attendees to notify.
        :param todo: The todo to include in the email.
        :param ics_attachment: An optional ICS attachment to include in the email.
        :return: True if the email(s) were sent successfully, False otherwise.
        """
        if self.send_mass_emails:
            body = template.build_todo_message(todo=todo, from_email=self.from_email,
                                               mass_email=self.send_mass_emails, attendee=None)
            subject = template.build_todo_subject(todo=todo, from_email=self.from_email,
                                                  mass_email=self.send_mass_emails, attendee=None)
            return self._send_email(subject=subject, body=body, attendees=attendees, ics_attachment=ics_attachment)
        else:
            failure_encountered = False
            for attendee in attendees:
                ...
                if not self._send_email(subject=subject, body=body, attendees=[attendee],
                                        ics_attachment=ics_attachment):
                    failure_encountered = True
            return not failure_encountered

    def _prepare_and_send_journal_email(self, template: MessageTemplate, attendees: List[Attendee],
                                        journal: EmailJournal, ics_attachment: Optional[ICSEmailAttachment] = None) -> bool:
        """
        Prepare the email message(s) and send them to the attendees for a journal notification.
        :param template: The message template to use for the email.
        :param attendees: The list of attendees to notify.
        :param journal: The journal entry to include in the email.
        :param ics_attachment: An optional ICS attachment to include in the email.
        :return: True if the email(s) were sent successfully, False otherwise.
        """
        if self.send_mass_emails:
            body = template.build_journal_message(journal=journal, from_email=self.from_email,
                                                  mass_email=self.send_mass_emails, attendee=None)
            subject = template.build_journal_subject(journal=journal, from_email=self.from_email,
                                                     mass_email=self.send_mass_emails, attendee=None)
            return self._send_email(subject=subject, body=body, attendees=attendees, ics_attachment=ics_attachment)
        else:
            failure_encountered = False
...
def _read_todo(vobject_data: str) -> EmailTodo:
    """
    Read the vobject item from the provided string and create an EmailTodo.
    """
    v_cal: vobject.base.Component = vobject.readOne(vobject_data)
    cal: Calendar = Calendar(vobject_item=v_cal)
    todo: Todo = cal.todo  # type: ignore

    return EmailTodo(
        todo=todo,
        ics_content=vobject_data,
        ics_file_name="todo.ics"
    )


def _read_journal(vobject_data: str) -> EmailJournal:
    """
    Read the vobject item from the provided string and create an EmailJournal.
    """
    v_cal: vobject.base.Component = vobject.readOne(vobject_data)
    cal: Calendar = Calendar(vobject_item=v_cal)
    journal: Journal = cal.journal  # type: ignore

    return EmailJournal(
        journal=journal,
        ics_content=vobject_data,
        ics_file_name="journal.ics"
    )
...
class Hook(BaseHook):
    def __init__(self, configuration):
        ...
        self.email_config = EmailConfig(
            ...
            # Todo templates (RFC 5545 VTODO)
            new_or_added_to_todo_template=MessageTemplate(
                subject="You have been assigned a task",
                body=self.configuration.get("hook", "new_or_added_to_todo_template")
            ),
            deleted_or_removed_from_todo_template=MessageTemplate(
                subject="A task you were assigned has been deleted",
                body=self.configuration.get("hook", "deleted_or_removed_from_todo_template")
            ),
            updated_todo_template=MessageTemplate(
                subject="A task you are assigned has been updated",
                body=self.configuration.get("hook", "updated_todo_template")
            ),
            # Journal templates (RFC 5545 VJOURNAL)
            new_journal_template=MessageTemplate(
                subject="A journal entry has been shared with you",
                body=self.configuration.get("hook", "new_journal_template")
            ),
            deleted_journal_template=MessageTemplate(
                subject="A journal entry has been deleted",
                body=self.configuration.get("hook", "deleted_journal_template")
            ),
            updated_journal_template=MessageTemplate(
                subject="A journal entry has been updated",
                body=self.configuration.get("hook", "updated_journal_template")
            )
        )
        logger.info(
    ...
    def _process_event_and_notify(self, notification_item: HookNotificationItem) -> None:
        """
        Process the notification item and send an email notification.
        Handles VEVENT, VTODO, and VJOURNAL components (RFC 5545/RFC 6047).
        ...
        if notification_type == HookNotificationItemTypes.CPATCH:
            ...
            # Determine the component type and route accordingly
            component_type = get_ics_component_type(new_item_str)
            if component_type == 'VEVENT':
                self._process_event_upsert(new_item_str, previous_item_str)
            elif component_type == 'VTODO':
                self._process_todo_upsert(new_item_str, previous_item_str)
            elif component_type == 'VJOURNAL':
                self._process_journal_upsert(new_item_str, previous_item_str)
            else:
                logger.debug("Unsupported component type in ICS file, skipping notification.")
            return
            ...
            # Determine the component type and route accordingly
            component_type = get_ics_component_type(deleted_item_str)
            if component_type == 'VEVENT':
                self._process_event_delete(deleted_item_str)
            elif component_type == 'VTODO':
                self._process_todo_delete(deleted_item_str)
            elif component_type == 'VJOURNAL':
                self._process_journal_delete(deleted_item_str)
            else:
                logger.debug("Unsupported component type in ICS file, skipping notification.")
            return
    ...
    def _process_event_upsert(self, new_item_str: str, previous_item_str: Optional[str]) -> None:
        """Process VEVENT upsert notification."""
    ...
    def _process_event_delete(self, deleted_item_str: str) -> None:
        """Process VEVENT delete notification."""
    ...
    def _process_todo_upsert(self, new_item_str: str, previous_item_str: Optional[str]) -> None:
        """Process VTODO upsert notification (RFC 5545 §3.6.2)."""
        email_todo: EmailTodo = _read_todo(vobject_data=new_item_str)  # type: ignore
        if not email_todo or not email_todo.todo:
            logger.error("Failed to read todo from new content: %s", new_item_str)
            return

        # Check if due date is in the past (similar to event end time check)
        if email_todo.todo.datetime_due and email_todo.todo.datetime_due.time:
            due_time = email_todo.todo.datetime_due.time
            now = datetime.now(due_time.tzinfo) if due_time.tzinfo else datetime.now()
            if due_time < (now - timedelta(minutes=1)):
                logger.warning("Todo due time is in the past, skipping notification for todo: %s",
                               email_todo.todo.uid)
                return

        attendees = email_todo.todo.attendees
        if not attendees:
            logger.debug("No attendees on todo, skipping notification.")
            return

        if not previous_item_str:
            # New todo - notify all attendees
            logger.debug("New todo detected, sending notifications to all attendees.")
            email_success: bool = self.email_config.send_added_todo_email(
                attendees=attendees,
                todo=email_todo
            )
            if not email_success:
                logger.error("Failed to send some or all added todo notifications for todo: %s",
                             email_todo.todo.uid)
            return

        # Update to existing todo - send update notification
        previous_todo = read_ics_todo(contents=previous_item_str)
        ...
        if not email_success:
            logger.error("Failed to send some or all updated todo notifications for todo: %s",
                         email_todo.todo.uid)

    def _process_todo_delete(self, deleted_item_str: str) -> None:
        """Process VTODO delete notification."""
        email_todo: EmailTodo = _read_todo(vobject_data=deleted_item_str)  # type: ignore
        if not email_todo or not email_todo.todo:
            logger.error("Failed to read todo from deleted content")
            return

        attendees = email_todo.todo.attendees
        if not attendees:
            logger.debug("No attendees on deleted todo, skipping notification.")
            return

        email_success: bool = self.email_config.send_deleted_todo_email(
            attendees=attendees,
            todo=email_todo
        )
        if not email_success:
            logger.error("Failed to send some or all deleted todo notifications for todo: %s",
                         email_todo.todo.uid)

    def _process_journal_upsert(self, new_item_str: str, previous_item_str: Optional[str]) -> None:
        """Process VJOURNAL upsert notification (RFC 5545 §3.6.3)."""
        email_journal: EmailJournal = _read_journal(vobject_data=new_item_str)  # type: ignore
        if not email_journal or not email_journal.journal:
            logger.error("Failed to read journal from new content: %s", new_item_str)
            return

        attendees = email_journal.journal.attendees
        if not attendees:
            logger.debug("No attendees on journal, skipping notification.")
            return

        if not previous_item_str:
            # New journal - notify all attendees
            logger.debug("New journal detected, sending notifications to all attendees.")
            email_success: bool = self.email_config.send_added_journal_email(
                attendees=attendees,
                journal=email_journal
            )
            if not email_success:
                logger.error("Failed to send some or all added journal notifications for journal: %s",
                             email_journal.journal.uid)
            return

        # Update to existing journal - send update notification
        previous_journal = read_ics_journal(contents=previous_item_str)
        ...
        if not email_success:
            logger.error("Failed to send some or all updated journal notifications for journal: %s",
                         email_journal.journal.uid)

    def _process_journal_delete(self, deleted_item_str: str) -> None:
        """Process VJOURNAL delete notification."""
        email_journal: EmailJournal = _read_journal(vobject_data=deleted_item_str)  # type: ignore
        if not email_journal or not email_journal.journal:
            logger.error("Failed to read journal from deleted content")
            return

        attendees = email_journal.journal.attendees
        if not attendees:
            logger.debug("No attendees on deleted journal, skipping notification.")
            return

        email_success: bool = self.email_config.send_deleted_journal_email(
            attendees=attendees,
            journal=email_journal
        )
        if not email_success:
            logger.error("Failed to send some or all deleted journal notifications for journal: %s",
                         email_journal.journal.uid)
