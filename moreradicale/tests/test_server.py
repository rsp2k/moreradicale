...
from moreradicale import config, server
from moreradicale.tests import BaseTest
from moreradicale.tests.helpers import configuration_to_dict, get_file_path
...
class TestBaseServerRequests(BaseTest):
    ...
    def test_command_line_interface(self, with_bool_options=False) -> None:
        ...
        p = subprocess.Popen(
            [sys.executable, "-m", "moreradicale"] + config_args,
    ...
    def test_wsgi_server(self) -> None:
        ...
        env["MORERADICALE_CONFIG"] = config_path
        ...
        p = subprocess.Popen([
            ...
            "moreradicale:application"], env=env)
        try:
